...
import LiveScoreClient from './live/LiveScoreClient';
...
export default async function LiveScorePage(props: { searchParams: Promise<{ roundId?: string }> }) {
    ...
    // 8. Redirect to ensure ID is in URL (Updated to redirect to root instead of /live)
    if (!roundIdFromUrl && activeRound) {
        return redirect(`/?roundId=${activeRound.id}`);
    }
