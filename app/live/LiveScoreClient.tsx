...
import { Bird, Copy, Mail, Send, ChevronDown } from 'lucide-react';
...
export default function LiveScoreClient({
    ...
    // Flag to prevent auto-select from overwriting restored data
    const hasAttemptedRestore = useRef(false);

    // Consolidated Selection Initialization
    useEffect(() => {
        if (!liveRoundId || typeof window === 'undefined') return;
        ...
        const performInitialization = () => {
            ...
            if (saved) {
                try {
                    ...
                    const allAvailablePlayers = [...allPlayers, ...guestPlayers];
                    ...
                    if (restored.length > 0) {
                        console.log("Group selection restored from LS:", restored.map(p => p.name));
            ...
            // 2. Admin Fallback (Auto-select ALL in round)
            if (isAdmin && initialRound?.players) {
                const allRoundPlayers: Player[] = [];
                initialRound.players.forEach((p: any) => {
                    const pid = p.is_guest ? p.id : p.player?.id;
                    if (pid) {
                        allRoundPlayers.push({
                            id: pid,
                            name: p.is_guest ? (p.guest_name || 'Guest') : p.player.name,
                            index: p.is_guest ? p.index_at_time : p.player.index,
                            preferred_tee_box: p.is_guest ? null : p.player.preferred_tee_box,
                            isGuest: !!p.is_guest,
                            liveRoundData: { tee_box_name: p.tee_box_name, course_hcp: p.course_handicap }
                        });
                    }
                });
                if (allRoundPlayers.length > 0) {
                    console.log("Admin fallback: Selecting everyone in round");
            ...
            // 3. Auto-select Current User (Last Resort)
            if (currentUserId && initialRound?.players && !hasAttemptedRestore.current) {
                const me = initialRound.players.find((p: any) =>
                    (p.player && p.player.id === currentUserId) || (p.is_guest && p.id === currentUserId)
                );
                if (me) {
                    const meAsPlayer: Player = {
                        id: me.is_guest ? me.id : me.player.id,
                        name: me.is_guest ? (me.guest_name || 'Guest') : me.player.name,
                        index: me.is_guest ? me.index_at_time : me.player.index,
                        preferred_tee_box: me.is_guest ? null : me.player.preferred_tee_box,
                        isGuest: !!me.is_guest,
                        liveRoundData: { tee_box_name: me.tee_box_name, course_hcp: me.course_handicap }
                    };
                    console.log("Initial load: Auto-selecting current user");
    ...
    }, [liveRoundId, allPlayers, guestPlayers, initialRound, currentUserId, isAdmin]);
    ...
    useEffect(() => {
        ...
        const hasLocationChanged = (newLat: number, newLon: number): boolean => {
            ...
            const phi1 = lastLocation.latitude * Math.PI / 180;
            const phi2 = newLat * Math.PI / 180;
            const deltaPhi = (newLat - lastLocation.latitude) * Math.PI / 180;
            const deltaLambda = (newLon - lastLocation.longitude) * Math.PI / 180;
    ...
    // Load initialization and guest players
    useEffect(() => {
    ...
    }, [initialRound?.id, initialRound?.players]);
    ...
    // Redundant auto-select removed ( consolidated above )
    ...
    useEffect(() => {
        ...
        const currentGps = searchParams.get('gps');
        const gpsStr = isGPSEnabled.toString();

        if (currentHole === activeHole.toString() && currentGps === gpsStr) return;

        const params = new URLSearchParams(searchParams.toString());
        params.set('hole', activeHole.toString());
        params.set('gps', gpsStr);
        router.replace(`?${params.toString()}`, { scroll: false });

        // Also sync to cookie for initial load
        Cookies.set('gps_enabled', gpsStr, { expires: 365, path: '/' });
    }, [activeHole, isGPSEnabled, liveRoundId, searchParams, router]);
    ...
    const handleAddPlayers = async (scoringList: any[], inRoundList: any[]) => {
        const scoringMapped = scoringList.map(p => ({
            id: p.id,
            name: p.name,
            index: p.index ?? 0,
            preferred_tee_box: p.preferred_tee_box ?? null,
            phone: p.phone,
            player_id: p.player_id,
            isGuest: p.isGuest || p.id.startsWith('guest-')
        }));

        const inRoundMapped = inRoundList.map(p => ({
            id: p.id,
            name: p.name,
            index: p.index ?? 0,
            preferred_tee_box: p.preferred_tee_box ?? null,
        ...
        const inRoundIds = new Set(inRoundMapped.map(p => p.id));

        // 1. Update LOCAL Scoring selection immediately
        setSelectedPlayers(scoringMapped);
        ...
        // 2. Add New Players to Round (on server)
        const regularInRound = inRoundMapped.filter(p => !p.isGuest);
        for (const player of regularInRound) {
            const existingLrPlayer = initialRound?.players?.find((p: any) => p.player?.id === player.id);
            if (!existingLrPlayer) {
                const teeBox = getPlayerTee(player);
                if (liveRoundId && teeBox?.id) {
                    console.log("Adding player to round leaderboard:", player.name);
        ...
        // 3. Remove Players dropped from BOTH (Explicit Drop)
        if (liveRoundId && initialRound?.players) {
            const vetoedPlayers: Player[] = [];

            // Check all players currently in the round on server
            for (const lrPlayer of initialRound.players) {
                const pid = lrPlayer.is_guest ? lrPlayer.id : lrPlayer.player?.id;
                if (pid && !inRoundIds.has(pid)) {
                    // Player was in the round but is now deselected for both Leaderboard and Scoring
                    console.log("Removing player from round entirely:", pid);
                    ...
                    try {
                        ...
                        if (!removeResult.success) {
                            if (removeResult.error && (removeResult.error.includes('does not exist') || removeResult.error.includes('Record to delete'))) {
                                ...
                                // If they are a regular player, we might want to restore them
                                if (!lrPlayer.is_guest && lrPlayer.player) {
                                    vetoedPlayers.push({
                                        id: lrPlayer.player.id,
                                        name: lrPlayer.player.name,
                                        index: lrPlayer.player.index,
                                        preferred_tee_box: lrPlayer.player.preferred_tee_box
                                    });
                                }
                            }
                        }
            ...
            // 4. Sync Scorer assignments to DB
            if (currentUserId) {
                await updatePlayerScorers({
                    liveRoundId: liveRoundId,
                    scorerId: currentUserId,
                    playerIds: scoringMapped.map(p => p.id)
                });
            }
    ...
    */
    ...
    const rankedPlayers = useMemo(() => {
        return summaryPlayers.map(player => {
            ...
            // 0. Primary Sort: "Has Scorer" vs "No Scorer" (Show "No Score Keeper" at bottom)
            if (a.scorerId && !b.scorerId) return -1;
            if (!a.scorerId && b.scorerId) return 1;

            // 1. Secondary Sort: Total Net (Ascending)
    ...
    return (
        <div className="w-full h-full min-h-screen bg-gray-50 pb-1 text-zinc-900">


            <main className="w-full h-full p-1 space-y-1">
                    ...
                    <div className="bg-white rounded-xl p-1 border border-zinc-200 shadow-xl flex flex-col justify-center space-y-1">
                            ...
                            <div className="flex gap-1">
                                ...
                                {isAdmin && liveRoundId && (
                                    <button
                                        ...
                                        className="bg-red-600 text-white text-lg font-black p-1 rounded-xl hover:bg-red-700 transition-all shadow-md active:scale-95 uppercase tracking-widest"
                                    >
                                        Delete
                                    </button>
                                )}
                        ...
                        <div className="relative">
                            <button
                                ...
                                className="w-full px-1 py-1 mt-1 text-2xl bg-black text-white rounded-xl font-bold flex justify-between items-center transition-all active:scale-[0.99] border border-black"
                                title="Select Round"
                            >
                                        ...
                                        {allLiveRounds.map((round) => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`w-full text-left px-2 py-2 text-2xl transition-colors border-b border-zinc-900 last:border-0 ${isSelected ? 'bg-zinc-800 text-white font-black' : 'text-white/50 hover:bg-zinc-900'
                                                        }`}
                                                >
                                                    <div className="flex justify-between items-center">
                    ...
                    <div className="bg-white/80 backdrop-blur-xl rounded-xl p-1 border border-zinc-200 shadow-xl">
                        <div className="flex justify-between items-start">
                            <div className="flex-1">
                                ...
                                <div className="flex flex-nowrap gap-x-1 text-2xl text-gray-500 mt-1 overflow-x-auto">
                                    ...
                                    {(() => {
                                        ...
                                        return teeIndicator && <span className="px-1 py-0.5 rounded text-xl font-bold bg-white text-black border border-black whitespace-nowrap">{teeIndicator}</span>;
                                    })()}
                                </div>
                            </div>
                                ...
                                {true && ( // Always show these buttons for navigation
                                    <div className="flex flex-col gap-1">
                                        <button
                                            ...
                                            className="bg-black text-white border border-black text-lg font-black px-1 py-1 rounded-xl hover:bg-zinc-800 transition-all shadow-md active:scale-95 uppercase tracking-widest"
                                        >
                                            Players
                                        </button>
                                        <button
                            ...
                            <div className="flex justify-between items-center border-b border-zinc-100 pb-1">
                                <div className="flex items-center gap-1">
                                    <button
                                        ...
                                        className={`px-1 py-1 rounded-xl text-2xl font-black transition-all shadow-md active:scale-95 uppercase tracking-widest ${isGPSEnabled
                                ...
                                <button
                                    ...
                                    className="px-1 py-1 bg-black text-white rounded-xl text-lg font-black transition-all hover:bg-zinc-800 uppercase tracking-widest shadow-md"
                                >
                                    Details
                                </button>
                            </div>
                                ...
                                <div className="min-h-[140px] flex flex-col justify-center">
                                    ...
                                    {(() => {
                                        ...
                                        if (!userLocation) {
                                            return (
                                                <div className="bg-gray-100 text-gray-500 p-1 rounded-full border-2 border-dashed border-gray-300 text-center mb-1 shadow-inner">
                                                    <p className="font-medium text-2xl animate-pulse py-1">🛰️ Waiting for GPS...</p>
                                                </div>
                                            );
                                        }
                                        ...
                                        if (!currentHole?.latitude || !currentHole?.longitude) {
                                            return (
                                                <div className="bg-yellow-50 text-yellow-700 p-1 rounded-full text-center mb-1 shadow-inner border-2 border-yellow-400">
                                                    <p className="font-medium text-2xl py-1">📍 Coordinates missing for Hole {activeHole}</p>
                                                </div>
                                            );
                                        }
                                        ...
                                        const renderElement = (side: 'LEFT' | 'RIGHT', num: number, positionClass: string) => {
                                            ...
                                            return (
                                                <div className={`absolute ${positionClass} flex items-center gap-1 text-white text-3xl font-extrabold z-10 drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]`}>
                                                    {side === 'LEFT' ? (
                                                        <>
                                                            {Numbers}
                                                            {Icons}
                                        ...
                                        return (
                                            <div className="bg-green-600 text-white w-full mx-auto p-1 rounded-xl text-center mb-1 border-2 border-black shadow-inner relative overflow-hidden">
                                                ...
                                                <p className="font-black text-8xl leading-none flex items-center justify-center pt-1 pb-1">
                                                    {dist || (dist === 0 ? '0' : '--')}
                                                </p>
                                            </div>
                                        );
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="flex flex-col items-center gap-0 leading-none">
                                                <span className="text-2xl font-black italic tracking-tighter">{hole.holeNumber}</span>
                                                <span className="text-lg font-black">{hole.par}</span>
                                            </div>
                                        </button>
                                    );
                                })}
                        ...
                        <div id="scoring-section" className="bg-white/80 backdrop-blur-xl rounded-xl p-1 border border-zinc-200 shadow-xl space-y-1">
                            <div className="flex justify-between items-center border-b border-zinc-100 pb-1">
                                <div className="flex items-center gap-2">
                                    <h2 className="text-2xl font-black text-zinc-900 italic uppercase tracking-tighter">Players ({effectiveScoringPlayers.length})</h2>
                                                ...
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    <div className="flex items-center gap-1">
                                                        <button
                                                            ...
                                                            disabled={index === 0}
                                                            className={`w-8 h-8 rounded-full flex items-center justify-center shadow-md active:scale-95 transition-all ${index === 0 ? 'bg-zinc-50 text-zinc-200 cursor-not-allowed' : 'bg-white border-2 border-zinc-900 text-black hover:bg-zinc-50'}`}
                                                            title="Move Up"
                                                        >
                                                            <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="4"><path d="M18 15l-6-6-6 6" /></svg>
                                                        </button>
                                                        <button
                                                            onClick={() => movePlayerOrder(index, 'down')}
                                                            disabled={index === effectiveScoringPlayers.length - 1}
                                                            className={`w-8 h-8 rounded-full flex items-center justify-center shadow-md active:scale-95 transition-all ${index === effectiveScoringPlayers.length - 1 ? 'bg-zinc-50 text-zinc-200 cursor-not-allowed' : 'bg-white border-2 border-zinc-900 text-black hover:bg-zinc-50'}`}
                                                            title="Move Down"
                                                        >
                                                            <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="4"><path d="M6 9l6 6 6-6" /></svg>
                                                        </button>
                                                        {(player.isGuest || player.id.startsWith('guest-')) && (
                                                            <button
                                                                ...
                                                                className="w-8 h-8 rounded-full bg-zinc-800 flex items-center justify-center text-white text-xl hover:bg-blue-600 transition-all shadow-lg border-2 border-black"
                                                                title="Edit Guest"
                                                            >
                                                                ✏️
                                                            </button>
                            ...
                            {effectiveScoringPlayers.length === 0 && (
                                <div className="py-1 text-center bg-gray-50 rounded-xl border border-dashed border-gray-300 mb-1">
                                    <p className="text-gray-500 font-bold text-xl mb-1">No players selected for scoring.</p>
                                    <button
                                        onClick={() => setIsPlayerModalOpen(true)}
                                        className="bg-black text-white px-1 py-1 rounded-xl font-bold text-lg shadow-md active:scale-95 transition-all"
                                    >
                                        Add Players / Join
                                    </button>
                                </div>
                    ...
                    <div className="fixed inset-0 z-[300] bg-gray-50 overflow-y-auto">
                        ...
                        <div className="bg-white shadow-sm sticky top-0 z-10 px-1 py-3 border-b border-gray-200">
                            <div className="flex items-center justify-between">
                                <h1 className="text-2xl font-bold text-gray-900 tracking-tight text-left ml-3">Round Stats</h1>
                                <button
                                    onClick={() => setIsStatsModalOpen(false)}
                                    className="px-1 py-2 bg-black text-white rounded-xl text-2xl font-bold hover:bg-gray-800 transition-colors mr-3"
                                >
                                    Close
                                </button>
                            </div>
                                    ...
                                    {birdieLeaders.length > 0 ? (
                                        birdieLeaders.map(player => (
                                            <div key={player.id} className="flex justify-between items-center bg-green-50 rounded-lg p-2">
                                                ...
                                                <span className="text-2xl font-black text-green-700">{player.birdieCount}</span>
                                            </div>
                                        ))
                                    ) : (
                                    ...
                                    {eagleLeaders.length > 0 ? (
                                        eagleLeaders.map(player => (
                                            <div key={player.id} className="flex justify-between items-center bg-yellow-50 rounded-lg p-2">
                                                <span className="text-xl font-bold text-gray-900">{player.name}</span>
                                                <span className="text-2xl font-black text-yellow-700">{player.eagleCount}</span>
                                            </div>
                                        ))
                                    ) : (
                        ...
                        >
                            ...
                            <div className="bg-white text-black rounded-2xl px-6 py-4 shadow-2xl flex flex-col items-center max-w-sm mx-4 border-4 border-green-500">
                                ...
                                <div className="text-2xl font-bold text-gray-900 text-center mb-4 w-full">
                                    {[...birdiePlayers].sort((a, b) => b.totalBirdies - a.totalBirdies).map((player, index) => (
                                        <div key={index} className="mb-2 last:mb-0 border-b last:border-0 border-gray-100 pb-2 last:pb-0">
                                            ...
                                            <div className="text-2xl text-green-600 font-bold leading-tight">
                                ...
                                <button
                                    ...
                                    className="w-full bg-black text-white rounded-xl py-2 text-xl font-bold hover:bg-gray-800 transition-colors shadow-md active:scale-95"
                                >
                                    Close
                                </button>
                            </div>
                            ...
                            <div className="bg-white text-black rounded-2xl px-6 py-4 shadow-2xl flex flex-col items-center max-w-sm mx-4 border-4 border-yellow-400">
                                ...
                                <div className="text-2xl font-bold text-gray-900 text-center mb-4 w-full">
                                    {[...eaglePlayers].sort((a, b) => b.totalEagles - a.totalEagles).map((player, index) => (
                                        <div key={index} className="mb-2 last:mb-0 border-b last:border-0 border-gray-100 pb-2 last:pb-0">
                                            <div className="leading-tight">{player.name}</div>
                                            <div className="text-2xl text-yellow-600 font-bold leading-tight">
