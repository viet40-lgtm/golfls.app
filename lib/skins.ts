...
export function calculateSkins(
    ...
    if (!participantIds || !Array.isArray(participantIds)) {
        console.warn("calculateSkins: participantIds is not an array", participantIds);
        return null;
    }
    ...
    activePlayers.forEach(p => {
        ...
        // FOR SKINS: Par 3 holes NEVER receive strokes. They are played as Gross Only.
        // We filter them out of the allocation sequence.
        const allocationHoles = holes.filter(h => h.par !== 3).sort((a, b) => a.difficulty - b.difficulty);

        const allocation: Record<number, number> = {};
        holes.forEach(h => allocation[h.number] = 0);

        let strokesRemaining = diff;
        if (allocationHoles.length > 0) {
            while (strokesRemaining > 0) {
                for (const hole of allocationHoles) {
